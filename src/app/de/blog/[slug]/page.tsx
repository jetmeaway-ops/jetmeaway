...
/**
 * German blog post route — /de/blog/<slug>.
 *
 ...
 * addressable URL plus an hreflang pair to be indexed as German content.
 *
 * The slug is shared with the English original — it is the URL and is
 * never translated — so /blog/<slug> and /de/blog/<slug> are the same
...
export function generateStaticParams() {
  return getAllPostSlugs('de').map(slug => ({ slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    // Canonical points at this German URL (not the English one) so the
    // translation is indexed in its own right; `languages` carries the
    // hreflang pair back to English plus x-default.
    alternates: blogAlternates(post.slug, 'de'),
    openGraph: {
      title: post.title,
      description: post.excerpt,
      url: postUrl(post.slug, 'de'),
      type: 'article',
      locale: 'de_DE',
...
export default async function GermanBlogPostPage({
  ...
  // Keep the reader inside German: sibling-post links in the body point
  // at /de/blog/… wherever that translation exists.
  const localisedPost = {
    ...post,
    content: localiseInternalBlogLinks(post.content, 'de'),
  };

  // Related posts come from the German corpus so the "Read next" cards
  // are German too.
  const relatedPosts = pickRelatedPosts(post, getAllPosts('de'));

  return (
    <>
      <Header />
      <BlogPostArticle post={localisedPost} relatedPosts={relatedPosts} locale="de" />
      <Footer />
    </>
  );
}
