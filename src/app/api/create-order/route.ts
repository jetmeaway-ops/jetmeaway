...
<body style="margin:0;padding:0;background:#F8FAFC;font-family:'Helvetica Neue',Arial,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 20px;">
    ...
    <div style="background:#fff;border:1px solid #E8ECF4;border-radius:16px;padding:20px;margin-bottom:24px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td style="padding:6px 0;font-size:14px;font-weight:700;color:#1A1D2B;text-align:right;">&pound;${(totalPerPerson - MARKUP_GBP).toFixed(2)}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td style="padding:6px 0;font-size:14px;font-weight:700;color:#1A1D2B;text-align:right;">&pound;${totalPerPerson.toFixed(2)}</td>
        </tr>
        ${passengers.length > 1 ? `
        <tr>
          <td style="padding:6px 0;font-size:14px;color:#5C6378;">&times; ${passengers.length} passengers</td>
          <td style="padding:6px 0;font-size:14px;font-weight:700;color:#1A1D2B;text-align:right;">&pound;${totalAll.toFixed(2)}</td>
        </tr>` : ''}
        <tr>
    ...
    <div style="text-align:center;padding:16px 0;border-top:1px solid #E8ECF4;">
      ...
      <p style="font-size:11px;color:#B0B8CC;margin:0;">JetMeAway &middot; jetmeaway.co.uk &middot; Your AI-powered travel scout</p>
    </div>
...
/* ═══════════════════════════════════════════════════════════════════════════
   MAIN HANDLER — POST /api/create-order
   ═══════════════════════════════════════════════════════════════════════════ */

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { offerId, passengers, paymentIntentId, sessionId } = body;

    // passengers: [{ id, given_name, family_name, born_on, gender, email?, phone? }]
    if (!offerId || !passengers?.length || !paymentIntentId) {
      return NextResponse.json(
        { error: 'Missing offer ID, passenger details, or payment intent' },
        { status: 400 },
      );
    }
    ...
    const confirmRes = await fetch(
      `https://api.duffel.com/payments/payment_intents/${paymentIntentId}/actions/confirm`,
      {
        method: 'POST',
        headers: {
    ...
    if (!confirmRes.ok) {
      ...
      console.error('Payment confirm error:', confirmRes.status, errText);

      let message = 'Payment confirmation failed. Your card was not charged.';
      try {
        const errJson = JSON.parse(errText);
        message = errJson.errors?.[0]?.message || message;
      } catch {}

      return NextResponse.json({ error: message }, { status: confirmRes.status });
    }
    ...
    if (!orderRes.ok) {
      ...
      let message = 'Booking failed. Your payment has been received — please contact support.';
      try {
        const errJson = JSON.parse(errText);
        message = errJson.errors?.[0]?.message || message;
      } catch {}
    ...
    /* ── Step 3: Fetch documents (PDF ticket) ───────────────────────── */
    ...
    if (RESEND_KEY && leadPassenger?.email) {
      try {
        ...
        await resend.emails.send({
          ...
          subject: `Booking Confirmed — ${origin} to ${dest} | JetMeAway`,
          html: emailHtml,
        });
    ...
    /* ── Step 5b: Send Personal Scout SMS via Twilio ─────────────────── */
    ...
    /* ── Step 6: Save booking intent to KV ───────────────────────────── */
