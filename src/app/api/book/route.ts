...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validate: there must be at least one adult (infants must have a guardian)
    ...
    if (RESEND_KEY && leadPassenger?.email) {
      try {
        ...
        const extraRecipients: string[] = Array.isArray(body.ccEmails) ? body.ccEmails.filter((e: any) => typeof e === 'string' && e.includes('@')) : [];
        const toList = [leadPassenger.email, ...extraRecipients];
