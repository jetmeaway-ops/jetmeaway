...
| 1.0.3 | 4 (local) | 2026-04-24 | **ARCHITECTURAL PIVOT — native screens → WebView shell.** User feedback: "app is really ugly and non functanil / same as our early days website". The 7 RN screens (~2,100 LOC) had none of the site's booking pipeline, checkout, account, hotel detail, destinations pSEO, admin etc. Replaced with a single full-screen `react-native-webview` over `https://jetmeaway.co.uk/` — app now renders the live production site. Android back = WebView back; external domains (affiliate links, Stripe on partner domains) open in system browser via `expo-web-browser` to avoid trapping users. Pull-to-refresh wired. Old `src/screens/*` + `src/navigation/*` left on disk as dead code — archive/delete after 1.0.3 is green in Play Store. Next: `eas build -p android --profile production`, EAS remote counter will set the real versionCode. |
| 1.0.3 | 13 | 2026-04-24 (SUBMITTED ~21:33 / APPROVED + LIVE ~22:00 UK) | **🚀 LIVE on Play Store — sub-30-min review.** EAS build `9fd0449f-906a-...` → AAB uploaded to Production as `jetmeaway 1.0.3 (13)` / 53 MB on disk, 15.6 MB install size (−2.29 MB vs 1.0.2 after stripping native screens). Rollout 100%, all targeted countries. Release notes (en-GB): "Major redesign — the app now matches our website, with full hotel booking, checkout, account access, and all destinations. Faster, cleaner, and updates instantly with every site release." Quick checks passed → Google reviewed and approved within ~30 min — record speed for this account. WebView shell architecture validated end-to-end. 1 non-blocking warning: missing deobfuscation file (cosmetic, ANR/crash stacks won't auto-symbolicate). EAS auth issue fixed this session by creating fresh access token after clearing stale `EXPO_TOKEN="your_fresh_token"` placeholder env var. |
