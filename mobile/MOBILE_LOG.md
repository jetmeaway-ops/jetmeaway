...
| 1.0.3 | 13 | 2026-04-24 (SUBMITTED ~21:33 UK) | **SENT TO GOOGLE FOR REVIEW.** EAS build `9fd0449f-906a-...` → AAB uploaded to Production as `jetmeaway 1.0.3 (13)` / 53 MB on disk, 15.6 MB install size (−2.29 MB vs 1.0.2 after stripping native screens). Rollout 100%, all targeted countries. Release notes (en-GB): "Major redesign — the app now matches our website, with full hotel booking, checkout, account access, and all destinations. Faster, cleaner, and updates instantly with every site release." Status = "Changes in review". Quick checks running (~13 min), then Google review (≤7 days typical). 1 non-blocking warning: missing deobfuscation file (ANR/crash stacks won't auto-symbolicate — cosmetic). EAS auth issue fixed this session by creating fresh access token after clearing stale `EXPO_TOKEN="your_fresh_token"` placeholder env var. |
